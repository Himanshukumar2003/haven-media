...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($countryCode) && !preg_match("/^\+\d{1,4}$/", $countryCode)) $errors[] = "Invalid country code format.";
    if (!filter_var($website, FILTER_VALIDATE_URL)) $errors[] = "Enter a valid website URL.";
    if (empty($services)) $errors[] = "Please select a service.";
